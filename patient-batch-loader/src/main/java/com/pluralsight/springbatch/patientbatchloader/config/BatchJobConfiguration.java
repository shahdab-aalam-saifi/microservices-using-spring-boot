...
public class BatchJobConfiguration {
    ...
    public Job job(Step step) throws Exception {
        return ((JobBuilder) this.jobBuilderFactory.get("patient-batch-loader").validator(this.validator())).start(step).build();
    }
    ...
    public JobParametersValidator validator() {
        return new JobParametersValidator() {
            public void validate(JobParameters parameters) throws JobParametersInvalidException {
                String fileName = parameters.getString("patient-batch-loader.fileName");
                if (StringUtils.isBlank(fileName)) {
                    throw new JobParametersInvalidException("The patient-batch-loader.fileName parameter is required.");
                } else {
                    try {
                        Path file = Paths.get(BatchJobConfiguration.this.applicationProperties.getBatch().getInputPath() + File.separator + fileName);
                        if (Files.notExists(file, new LinkOption[0]) || !Files.isReadable(file)) {
    ...
    @Bean
    @StepScope
    public FlatFileItemReader<PatientRecord> reader(@Value("#{jobParameters['patient-batch-loader.fileName']}") String fileName) {
        return (new FlatFileItemReaderBuilder()).name("patient-item-reader").resource(new PathResource(Paths.get(this.applicationProperties.getBatch().getInputPath() + File.separator + fileName))).linesToSkip(1).lineMapper(this.lineMapper()).build();
    }
    ...
    public LineMapper<PatientRecord> lineMapper() {
        ...
        mapper.setFieldSetMapper((fieldSet) -> {
            return new PatientRecord(fieldSet.readString(0), fieldSet.readString(1), fieldSet.readString(2), fieldSet.readString(3), fieldSet.readString(4), fieldSet.readString(5), fieldSet.readString(6), fieldSet.readString(7), fieldSet.readString(8), fieldSet.readString(9), fieldSet.readString(10), fieldSet.readString(11), fieldSet.readString(12));
        });
    ...
    public JpaItemWriter<PatientEntity> writer() {
        ...
        writer.setEntityManagerFactory(this.batchEntityManagerFactory);
        return writer;
    }
}
